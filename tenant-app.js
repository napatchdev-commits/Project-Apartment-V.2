...
class TenantDBService {
  ...
  static async getPublicState() {
    ...
    return {
      settings: { apartmentName: data.apartmentName || '' },
...
class MyBillsApp {
  ...
  static async init() {
    ...
    loader.innerHTML = `
      ...
      <div style="font-size:0.9rem; color:#94a3b8;">ระบบผู้เช่า - ระบบจัดการหอพัก</div>
      <style>
  ...
  static renderLoginScreen() {
    ...
    const apartmentName = settings.apartmentName || 'ระบบจัดการหอพัก';
    ...
    return `
      <div class="tenant-card animate-fade-in" style="margin: auto; width: 100%; padding: 2rem;">
        ...
        <div style="margin-top:2.5rem; padding-top:1.25rem; border-top:1px solid #e2e8f0; text-align:center;">
          <p class="text-muted" style="font-size:0.82rem;">${settings.tel ? `ติดต่อสำนักงานหอพัก โทร. ${settings.tel}` : 'ติดต่อผู้ดูแลระบบสำหรับข้อมูลเพิ่มเติม'}</p>
        </div>
      </div>
    `;
  }
  ...
  static getHeaderTitleText() {
    const aptName = (this.state.settings && this.state.settings.apartmentName) || 'ระบบจัดการหอพัก';
  ...
  static openContactAdminModal() {
    ...
    const settings = (this.state && this.state.settings) || {};
    const adminPhone = settings.tel || settings.contactPhone || '';
    ...
    dialog.innerHTML = `
      ...
      <div class="modal-body" style="text-align:center;">
        ...
        <h4 style="font-size:1.1rem; font-weight:800; color:var(--text-main); margin-bottom:0.25rem;">
          ${settings.apartmentName || 'หอพัก'}${settings.apartmentName ? '' : ' <span style="font-size:0.75rem; color:#9ca3af; font-style:italic;">(ยังไม่ได้ตั้งค่า)</span>'}
        </h4>
        ...
        <div style="background:#f8fafc; border:1px solid var(--border-color); border-radius:var(--radius-lg); padding:1rem; margin-bottom:1.25rem; text-align:left; display:flex; flex-direction:column; gap:0.75rem;">
          <div style="display:flex; align-items:center; gap:0.75rem;">
            ...
            <div>
              ...
              <a href="tel:${adminPhone}" style="font-weight:800; font-size:1rem; color:var(--primary); text-decoration:none;">${adminPhone || '<span style="color:#9ca3af; font-style:italic;">ยังไม่ได้ตั้งค่าเบอร์โทร</span>'}</a>
            </div>
          </div>
          <div style="display:flex; align-items:center; gap:0.75rem;">
          ...
          <div style="display:flex; align-items:center; gap:0.75rem;">
            ...
            <div>
              ...
              <div style="font-weight:600; font-size:0.82rem; color:var(--text-sub);">${settings.address || '<span style="color:#9ca3af; font-style:italic;">ยังไม่ได้ตั้งค่าที่อยู่</span>'}</div>
            </div>
          </div>
        </div>
      ...
      <div class="modal-body" style="padding:1rem;">
        ...
        <div id="modal-printable-bill-area" style="background:#ffffff; border:1px solid #e2e8f0; border-radius:12px; padding:1.25rem; font-family:sans-serif; color:#0f172a;">
          <div style="display:flex; justify-content:space-between; border-bottom:2px solid #0f172a; padding-bottom:0.75rem; margin-bottom:1rem;">
            <div>
              <h2 style="font-size:1.25rem; font-weight:800; color:#0f172a;">${(this.state.settings && this.state.settings.apartmentName) || 'ระบบจัดการหอพัก'}</h2>
              <p style="font-size:0.78rem; color:#64748b; margin-top:0.2rem; line-height:1.3;">${(this.state.settings && this.state.settings.address) || ''}</p>
            </div>
          ...
          <div style="text-align:center; margin-top:1rem; padding-top:0.75rem; border-top:1px dashed #cbd5e1;">
            <p style="font-size:0.8rem; color:#059669; font-weight:700;">ขอบพระคุณที่เลือกใช้บริการ${(this.state.settings && this.state.settings.apartmentName) ? ` ${this.state.settings.apartmentName}` : ''}</p>
          </div>
        </div>
      ...
      <div class="modal-body" style="padding:1rem;">
        ...
        <div id="modal-printable-bill-area" style="background:#ffffff; border:1px solid #cbd5e1; border-radius:12px; padding:1.25rem; font-family:sans-serif; color:#0f172a;">
          <div style="display:flex; justify-content:space-between; align-items:flex-start; border-bottom:2px solid #2563eb; padding-bottom:0.75rem; margin-bottom:1rem;">
            <div>
              <h2 style="color:#1e40af; font-size:1.25rem; font-weight:800; margin-bottom:0.2rem;">${(this.state.settings && this.state.settings.apartmentName) || 'ระบบจัดการหอพัก'}</h2>
              <p style="font-size:0.78rem; color:#475569; margin:0; line-height:1.3;">${(this.state.settings && this.state.settings.address) || ''}</p>
            </div>
            <div style="text-align:right;">
